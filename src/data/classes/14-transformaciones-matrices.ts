...
* **Sobre:** Una transformación es **sobreyectiva** (o "sobre") cuando **cada vector del espacio destino puede obtenerse como imagen de algún vector del espacio de partida**.  
...
* Si $T$ es **uno a uno**, ¿qué se puede afirmar acerca de $m$ y $n$?
      `.trim(),
      solution: String.raw`
**Caso 1: $T$ es sobre (sobreyectiva)** Ser sobreyectiva significa que la ecuación $A\mathbf{x} = \mathbf{b}$ siempre tiene solución para cualquier $\mathbf{b} \in \mathbb{R}^m$. En otras palabras, las columnas de $A$ generan todo el espacio $\mathbb{R}^m$, lo que implica que el **rango de $A$** (el número de columnas linealmente independientes) debe ser $m$.  
...
**Caso 2: $T$ es uno a uno (inyectiva)** Ser inyectiva significa que la ecuación homogénea $A\mathbf{x} = \mathbf{0}$ solo tiene la solución trivial $\mathbf{x} = \mathbf{0}$. Esto ocurre cuando las columnas de $A$ son **linealmente independientes**, lo que implica que el rango de $A$ es $n$.  
...
**Conclusión Final:** * Si $T$ es sobreyectiva, entonces $m \leq n$.
